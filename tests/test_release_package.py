...
PACKAGE = Path(__file__).resolve().parents[1] / "packages" / "x60_4_lh2_v7_guarded"
...
class ReleasePackageTest(unittest.TestCase):
    ...
    def test_identity_matches_root_reference_vector(self) -> None:
        ...
        self.assertAlmostEqual(result.curve_scale, 1.0286087671251791, places=14)
        self.assertAlmostEqual(result.run_scale, 0.9982154163968054, places=14)
        self.assertAlmostEqual(result.seed_scale, 1.0003429583633106, places=14)
        self.assertAlmostEqual(result.lowe_scale, 1.0, places=14)
        self.assertAlmostEqual(result.corrected_energy_gev, 0.575986276774677, places=14)

    def test_smooth_lowe_is_continuous_at_boundaries(self) -> None:
        epsilon = 1.0e-10
        for boundary in (0.8, 1.0):
